// Archived per-release notes and RELEASE_HISTORY. Managed by `npm run release:prepare`;
// the current release lives in ./release.ts. Keep archived consts above RELEASE_HISTORY.

import { RELEASE_NOTES } from "./release.js";

const RELEASE_1_13_1_NOTES = [
  "优化讲道台预览与显示设置：桌面端投影和手机预览并排等高、空间不足时自动换行，修复动态挂载后画面停在 0.3 倍的问题；重新排布字体与版心控件，新增多款渐变主题及可同步的文字颜色，版心边距统一作用于经文出处和正文。",
  "讲道台支持多人同时演示：任何成员都可发起「小组演示」并邀请特定成员观看；持有讲道授权的成员可发起面向全体的「全体演示」（发起时服务端复检授权），管理员可直接发起且权限确认期间不再误显示申请提示。每位讲道者各自维护队列与展示进度，互不影响。",
  "新增讲道演示观看入口与邀请横幅：受邀成员收到「X 邀请你观看讲道演示」横幅（可加入、稍后或不再提示），聊天头部入口图标列出所有可观看的演示并标注未观看数量。",
  "观众同一时间只能观看一场演示：观看中想切换时需「离开当前并加入」，主持人可在讲道台查看观众名单并移除观众；观众断线、刷新或登出即自动释放席位（讲道者断线演示继续进行），管理员可强制结束任意演示，演示结束时所有观众自动退出并收到提示。"
] as const

const RELEASE_1_13_0_NOTES = [
...
const RELEASE_1_12_12_NOTES = [
  ...
  "讲道队列支持自定义文字条目：可添加可选标题与正文（如讲章大纲），与经文条目一样支持排序、翻页与展示同步；字体子集已扩充至 GBK 全集以覆盖任意文字。",
  "讲道台桌面端改为双栏布局：左侧为队列与演示控制，右侧为 16:9 投影预览与手机预览两个实时画面；同步提示改为悬浮显示，不再顶动经文。"
] as const

const RELEASE_1_12_11_NOTES = [
  "管理端的聊天数据和用户数据导出改为 ZIP 包：用户导出包含全部头像文件，聊天导出包含消息和置顶引用的附件文件；导入同时接受新的 ZIP 包和旧版 JSON 文件，导入时自动还原头像与附件。",
  "讲道台从弹窗改为“负一屏”工作区：队列屏点击条目进入演示视图，讲道者看到的画面与观众端内容和大小比例完全一致，仅多一条控制栏（上一条/下一条、整节高亮与划线、选段划线、字体倍率、返回队列、结束展示）；字体倍率全端实时同步（0.7–1.6），经文舞台组件在讲道者与观众端之间共享。",
  "新增“讲道经文展示”：讲道者可在讲道台输入一个或一串经文出处、维护展示队列并推送当前条目给所有在线成员；观众端以全屏大字呈现，支持整节高亮/划线与选段划线的实时同步；成员可通过 `/申请演讲` 生成申请卡，由管理员在卡片上批准（24 小时/7 天/30 天/永久）或拒绝。"
] as const

const RELEASE_1_12_10_NOTES = [
...
const RELEASE_1_11_2_NOTES = [
  ...
  "频道列表保持综合频道置顶，其余频道按最新消息自动上移；每个频道（包括会客厅）可设置独立列表底色，未读位置改为账号级持久化并实时同步到同账号的其他设备。",
  "修复两字中文来访口令在创建或修改会客厅时仍被旧的四字符校验拦截的问题。"
] as const

const RELEASE_1_11_1_NOTES = [
  "会客厅口令提示明确支持中文，表单控件统一为主体字号；管理界面改为点击后才加载，并合并实时事件引起的重复频道请求，改善弱网体验，同时避免正式成员回收当前会客厅时被误退出。"
] as const

const RELEASE_1_11_0_NOTES = [
...
const RELEASE_1_9_4_NOTES = [
  ...
  "实时同步改增量推送：代祷打卡、代祷状态更新、AI 经文建议、撤回消息不再让频道里所有人整页重新加载，只推送变化的那一条；频道改名、成员变更也不再触发每个客户端重新拉取全部频道数据。代祷集中打卡时段的卡顿和转圈会明显减少。",
  "消息列表渲染减负：Markdown 解析、安全消毒、经文引用切分、语音波形重采样等每行消息的重型计算改为按消息缓存结果，播放语音、特效动画、打字提示出现时不再连带重算整个屏幕的消息，长列表滑动更顺、更省电。",
  "首屏脚本继续减重：Markdown、实时通信等基础库单独打包并长效缓存，发版后老用户只需下载真正变化的部分；历史版本更新记录改为打开「关于」页时才加载。首屏压缩后约 195KB。",
  "登录状态核对提速：服务器对每个请求的「查岗」结果做 30 秒缓存（退出登录、改密码、管理员改号删号立即失效），高并发时数据库压力明显降低。"
] as const

const RELEASE_1_9_3_NOTES = [
  "后台流量与服务器负载继续收敛：「正在输入」提示改为每 3 秒最多发一次（服务端另有 2 秒去抖）；听歌/读经/听节目的在线状态心跳只在状态真正变化时广播，空闲时不再全员推送；链接预览在服务器端缓存 30 分钟，同一链接不再每人每次都重新抓取。",
  "头像、聊天图片、文件、歌谱等资源改为长效缓存：每次打开聊天室不再为每张图回源核对。",
  "修复生产构建一直携带开发模式代码的问题，首屏脚本再减约 15%（压缩后约 198KB）。",
  "聊天图片改为缩略图传输：新上传的图片自动生成 480px 小图供气泡显示和预载（点开查看仍加载原图），服务器启动时会为历史图片补齐；头像上传统一缩到 256px，聊天背景限制在 2560px 以内——图片多的频道加载流量可省一个数量级。"
] as const

const RELEASE_1_9_2_NOTES = [
  "负一屏圣经多选经文优化：连续选中的经节高亮合并为连续一段，不再逐节断开；多选时恢复「发送」按钮，连续经节自动合并为一条消息整体发送（如选中创世记 1:1-3 发送为「创世记 1:1-3」一条，而非逐节三条），跨章或不连续的选区按段分别发送。",
  "负一屏圣经顶栏「聊天」与「目录」调换位置：「目录」在左上角，「聊天」在右上角。",
  "修复负一屏圣经阅读器在章首附近轻微上滑加载前一章时页面剧烈跳转的问题。",
  "代祷更新弹窗精简重做：去掉「划去选中文字」工具和说明文字，只保留一个输入框；修复在输入框里从右往左拖选文字时弹窗被误关闭的问题；更新动态时可以附上一张照片，照片随文字一起显示在卡片顶部、下次更新时一起下移进历史。",
  "发起代祷也可以附照片：代祷视图下从「更多 → 照片」选图会作为附件随代祷一起发送，卡片上直接显示。",
  "修复桌面端窗口较窄时代祷卡片过宽、挡住发消息人头像的问题。",
  "代祷卡片颜色可自定义：管理员在「管理面板 → 外观与体验 → 聊天室外观」里分别设置「自己的代祷卡片」和「别人的代祷卡片」颜色，保存后对所有成员即时生效。"
] as const

const RELEASE_1_9_1_NOTES = [
  "代祷卡片「更新最新动态」重做：点更新后直接书写一条全新的动态，不再要求在原全文上修改；新动态显示在卡片顶部，原有内容自动下移保留为历史动态（带时间和更新人，新的在上、旧的在下，最多保留 20 条），更新仍会作为最新消息推送给全员。"
] as const

const RELEASE_1_9_0_NOTES = [
  "启动明显提速：启动请求改为并行加载，未读数统计不再阻塞首屏；消息窗口按账号缓存在本机，再次打开聊天室时先即时显示缓存的最新消息，再在后台与服务器核对更新。",
  "首屏等待再减一轮：启动页现在只等外观和账号两个请求完成就进入聊天室（缓存消息立即可见），频道、消息、成员的联网核对全部改到后台进行，深链打开频道时会自动等频道列表就绪。",
  "启动请求全部并行化：首页消息与账号验证同时发出，不再排队等待；音乐库、播放状态改为后台并行加载，不再挡在聊天滚动和版本检查前面。",
  "启动流量大幅收敛：歌谱图片只预载当前恢复曲目（原为全部曲目全部歌谱页，查看时仍会按需加载）；消息图片缓存预热改为空闲时进行且只覆盖最新 30 张；链接预览请求从最多 40 个并发降为 3 个，避免挤占消息加载带宽。",
  "未读角标的启动核对从「每个频道各发一次完整消息请求」改为一次合并计数请求，频道越多提速越明显；弱网下接口请求超过 20 秒会自动中止并重试一次，不再出现长时间无响应的转圈。",
  "频道列表和消息列表的服务器查询从逐频道、逐条消息的多次查询改为批量查询：频道列表从每频道十余次查询降为固定几次，消息页内语音已读状态、歌谱歌词、代祷统计、分享歌单等附加数据均改为整页一次取回，消息多的频道打开更快。",
  "首屏脚本减半：PDF 查看器、歌谱内联预览、经文阅读区、音乐管理、管理后台等改为首次打开时才下载，首次进入聊天室需要加载的脚本从约 1.4MB 降到约 720KB（压缩后约 230KB），手机端解析也更快。",
  "传输体积大幅下降：服务器为接口响应和网页资源启用压缩传输，带内容指纹的静态资源改为一年长效缓存，重复打开不再需要逐文件回源核对。",
  "修复启动时先闪现较早消息、气泡跳动后才跳到最新消息的问题：冷启动直接锚定最新消息，加载中的阅读位置恢复可被及时中止，频道切换也不再闪现停在历史中部的缓存内容。",
  "修复 iPhone 长语音在约两分钟时被系统提前中断却仍静默发送的问题：录音期间会尽量保持屏幕唤醒，长录音不再使用 WebKit 易出错的分片模式；若浏览器仍意外中断，会明确提示当前预览只保留了前半段。",
  "修复 iPhone 12 聊天输入栏左侧语音按钮尺寸不一致造成的轻微错位，移动端两侧按钮现在与单行输入框保持 38px 方形对齐。"
] as const

const RELEASE_1_8_4_NOTES = [
  "音乐管理界面精简改版：顶部「音乐管理」一行新增「创建歌单」按钮；导航精简为「全部诗歌 / 我的收藏 / 歌单列表 / 待绑定资源」，不再平铺每个歌单，自己创建和收藏的歌单都收进「歌单列表」，点进去即可查看该歌单全部曲目；歌单详情工具行改为「从曲库添加 / 分享 / 重命名 / 删除」加排序，歌单信息（名称、曲目数、创建者）上移到按钮行与搜索行之间，搜索曲目独立成行；去掉网格/列表切换，曲目始终列表显示。",
  "修复歌单「从曲库添加」弹窗看不到底部按钮的问题：曲目较多时保存按钮被裁出可视区，现在弹窗新增曲目搜索，底部「完成」「取消」按钮始终可见。"
] as const

const RELEASE_1_8_3_NOTES = [
...
const RELEASE_1_8_0_NOTES = [
  ...
  "音乐小窗新增歌单来源显示与切换：队列区显示当前所在歌单名称（系统曲库显示「聊天室曲库」），点击「切换歌单」可弹出窗口选择聊天室曲库、收藏的曲目或任意歌单。",
  "音乐小窗的播放方式改为顺序/随机/单曲循环三个独立按钮，文字过长时自动滚动；定时停止的输入框缩窄到 3 个中文字符宽。",
  "所有用户现在都可以上传音乐；每首音乐都会标注上传者名称。"
] as const

const RELEASE_1_7_6_NOTES = [
  "频道列表新增微信式未读消息红点角标：其他频道收到新消息时实时显示未读条数（最多 99+），打开频道即清零，刷新页面后未读数仍会恢复。",
  "后台活动日志的听歌记录改为每次收听只记一条汇总：记录所听歌曲、是否听完、实际收听秒数与占比，不再每几秒写入一条进度记录；听歌相关的周期性请求也不再刷服务器请求日志。",
  "点击聊天消息头像现在可以直接打开该成员的操作菜单，与成员列表操作一致。",
  "修复手机地址栏伸缩或键盘弹出导致“哎呀”掉落文字卡在可视区外的问题：视口变化后已落地的文字会被拉回地板之上。"
] as const;

const RELEASE_1_7_5_NOTES = [
...
const RELEASE_1_7_2_NOTES = [
  ...
  "修复“友”的“最近收听”混入并未点选收听节目的问题：打开节目单时的随机自动播放不再写入收听记录，只有主动点播的节目才记录进度与历史（历史数据本就按账号隔离）。",
  "负一屏圣经顶部的“资”图标不再跳转外部链接，仅保留图标。"
] as const;

const RELEASE_1_7_1_NOTES = [
  "修复“友”良友节目列表停留在 2022 年 3 月旧节目的问题：上游良友随身听已改为动态接口，节目单改经其 JSON 接口抓取，与官方页面的“今日节目”保持一致。",
  "“友”良友节目新增“更多节目”标签：按分类浏览全部节目系列，点进任一系列即可播放其近期节目，音频与封面同样经聊天室服务器转发。",
  "修复“良友圣经学院”等系列封面破损的问题：封面改用上游统一的系列封面图，全部经服务器代理。",
  "点击“友”打开节目单时自动从“今日节目”随机播放一个节目；播放进度按账号记录在服务器，再次收听自动从上次位置续播，“最近收听”标签可回看并继续播放最近 20 条记录。",
  "节目单改为服务器每天 7:00 和 19:00 自动刷新，移除手动刷新按钮以减轻服务器压力。",
  "收听良友节目的状态与听歌曲一样在聊天室顶部滚动条实时显示。",
  "聊天顶栏的圣经、“歌”、“友”、“字”、成员图标排布更紧凑。"
] as const

const RELEASE_1_7_0_NOTES = [
...
const RELEASE_1_6_0_NOTES = [
  ...
  "修复音乐管理/歌单管理中上传 PDF 歌谱时客户端错误提示“只支持图片”的问题：客户端校验现在与后端保持一致，允许单独上传一个 PDF 或最多 20 页图片歌谱。",
  "修复音乐管理中点击“AI 补全资料”后，“写作背景”和“知识歌词”编辑框没有立即显示生成内容的问题：补全成功后直接把结果回填到对应编辑框，方便查看和继续编辑。",
  "修复从顶部展开播放器点击“音乐管理”进入歌单后，点击歌单任意位置管理器会意外关闭的问题：打开音乐管理器时同时收起展开播放器，避免其外部点击关闭逻辑误触。",
  "修复局域网 HTTP 访问（非 localhost、非 HTTPS）时点击播放音乐没有反应的问题：`crypto.randomUUID` 仅在安全上下文可用，播放会话编号改用带降级的生成方式；同隐患的自定义卷轴套件编号一并修复。",
...
const RELEASE_1_5_0_NOTES = [
  ...
  "程序缓存按版本隔离，登录与运行中继续检查服务器版本；升级只清理旧程序缓存，退出登录会清理当前会话的私有内容缓存。"
] as const;

const RELEASE_1_4_0_NOTES = [
...
const RELEASE_0_12_9_NOTES = [
  ...
  "修复 macOS 浏览器把兼容性平台名 MacIntel 当作设备名的问题；未来登录和旧记录展示统一识别为 Mac，iPadOS 兼容模式会识别为 iPad。"
] as const;

const RELEASE_0_12_8_NOTES = [
...
const RELEASE_0_10_2_NOTES = [
  "左侧收藏夹移至用户资料区上方并保持固定；频道过多时仅滚动频道列表，收藏夹和用户管理区不随之滚动。"
] as const;

const RELEASE_0_10_1_NOTES = [
...
const RELEASE_1_13_4_NOTES = [
  "讲道台可从队列页退出当前模式并重新选择小组或全体；全体模式不再显示或发送成员邀请。版心边距下限改为真正的 0%，经文内容可贴齐舞台左右边缘。",
  "演讲者预览区下方新增当前经文的章节上下文，并用绿边黄底标出正在投影的经节；观众可长按经文进入仅本机生效的上下文滚动视图，点按其他位置即可返回演示。"
] as const

const RELEASE_1_13_5_NOTES = [
  "讲道台队列改为先选择预览、再由“开始演示”明确推送，并可随时“结束演示”而保留队列；每张经文幻灯片默认按段落显示，可单独切回逐节换行，所有幻灯片可独立设置居中，纯文字默认居中且不再显示“文字分享”徽标。经文居中时出处移到正文末尾，显示设置新增同步行距调节；“下一页”改为普通网页文字预览，经文上下文默认连续成段。"
] as const

const RELEASE_1_13_6_NOTES = [
  "讲道台“经文上下文”简化为“上下文”，经卷与章节移到标题右侧并改用普通网页字体；上下滚动会按需连续加载本卷前后章节，“复位”按钮可随时把当前投影经文重新置于面板中间。"
] as const

const RELEASE_1_13_7_NOTES = [
  "圣经书房“经卷目录”按通行的九个目录分部为 66 卷标色：摩西五经、历史书、智慧书、大先知书、小先知书、福音书、使徒行传、书信和启示录；每张经卷卡同时标明分组，便于不依赖颜色辨识。",
  "修复手机和桌面聊天室消息位置不稳定的问题：进入频道、手动回到最新消息、图片或链接预览晚到、虚拟列表重新测量以及手机键盘或浏览器栏改变视口后，都会继续稳定守住最新消息；已取消的旧位置校正不再把页面拉回上方。"
] as const

const RELEASE_1_13_8_NOTES = [
  "经卷目录的分部颜色改为淡底色与细边框，不再使用顶部高饱和色条，也不在每张经卷卡上重复显示文体；目录标题下新增九类颜色索引。",
...
const RELEASE_1_15_0_NOTES = [
  "圣经书房新增当代译本修订版（CCB）：每个阅读窗格的工具栏可独立切换「和合本 / 当代译本」，支持两个版本并排对照阅读；全文检索跟随当前窗格所用译本，窗格版本随账号同步并在分享“打开的圣经”时保留。当代译本为 Biblica 开放资源（CC BY-SA 4.0），书末附有版权署名。聊天消息中的经文展开、讲道台经文、经文收藏与 AI 主题检索仍固定使用和合本。"
] as const

const RELEASE_1_16_0_NOTES = [
...
const RELEASE_2_0_0_NOTES = [
  "新增「我的故事」：可发布照片、语音和文字组合的个人时间线（不能只发文字），支持 HEIC 照片转换、图片浏览、语音播放、个人签名与按性别显示的「她的故事 / 他的故事 / TA的故事」。",
  "故事支持持久化点赞与评论：成员可点赞、发表或删除自己的评论，故事作者和管理员可管理评论；互动字号跟随聊天设置，并采用适配手机与桌面的紧凑布局。",
  "聊天顶栏在「友」右侧新增「我的故事」快捷入口，沿用「数算恩典」的星光图标，侧栏头像入口继续保留。"
] as const

const RELEASE_2_1_0_NOTES = [
  "顶栏星光入口默认打开「我们的故事」，按时间合流展示所有可见成员的故事；长按入口可直接查看「我的故事」，也可点击页首标题在两种视图间切换。",
  "新故事及本人故事收到的点赞、评论会实时点亮彩色星光并显示提醒，阅读后跨设备恢复普通状态；发布故事时会在当前聊天室留下“某某发了个故事”的系统提示。",
...
export const RELEASE_HISTORY = [
  ...
  },
  {
    version: "2.1.0",
    date: "2026-09-19",
    notes: RELEASE_2_1_0_NOTES
  },
  {
    version: "2.0.0",
    date: "2026-09-19",
    notes: RELEASE_2_0_0_NOTES
  },
  {
    version: "1.18.5",
    date: "2026-09-18",
    notes: RELEASE_1_18_5_NOTES
  },
  {
    version: "1.18.4",
    date: "2026-09-18",
    notes: RELEASE_1_18_4_NOTES
  },
  {
    version: "1.18.3",
    date: "2026-09-17",
    notes: RELEASE_1_18_3_NOTES
  },
  {
    version: "1.18.2",
    date: "2026-09-16",
  ...
  },
  {
    version: "1.18.1",
    date: "2026-09-15",
    notes: RELEASE_1_18_1_NOTES
  },
  {
    version: "1.18.0",
    date: "2026-09-15",
    notes: RELEASE_1_18_0_NOTES
  },
  {
    version: "1.17.1",
    date: "2026-09-08",
    notes: RELEASE_1_17_1_NOTES
  },
  {
    version: "1.17.0",
    date: "2026-09-07",
    notes: RELEASE_1_17_0_NOTES
  },
  {
    version: "1.16.1",
    date: "2026-09-04",
    notes: RELEASE_1_16_1_NOTES
  },
  {
    version: "1.16.0",
    date: "2026-09-04",
    notes: RELEASE_1_16_0_NOTES
  },
  {
    version: "1.15.0",
    date: "2026-09-03",
    notes: RELEASE_1_15_0_NOTES
  },
  {
    version: "1.14.4",
    date: "2026-09-03",
  ...
  },
  {
    version: "1.14.3",
    date: "2026-09-03",
    notes: RELEASE_1_14_3_NOTES
  },
  {
    version: "1.14.2",
    date: "2026-09-02",
    notes: RELEASE_1_14_2_NOTES
  },
  {
    version: "1.14.1",
    date: "2026-09-02",
  ...
  },
  {
    version: "1.13.8",
    date: "2026-08-31",
    notes: RELEASE_1_13_8_NOTES
  },
  {
    version: "1.13.7",
    date: "2026-08-31",
    notes: RELEASE_1_13_7_NOTES
  },
  {
    version: "1.13.6",
    date: "2026-08-30",
    notes: RELEASE_1_13_6_NOTES
  },
  {
    version: "1.13.5",
    date: "2026-08-30",
    notes: RELEASE_1_13_5_NOTES
  },
  {
    version: "1.13.4",
    date: "2026-08-30",
    notes: RELEASE_1_13_4_NOTES
  },
  {
    version: "1.13.3",
    date: "2026-08-29",
  ...
  },
  {
    version: "1.13.2",
    date: "2026-08-29",
    notes: RELEASE_1_13_2_NOTES
  },
  {
    version: "1.13.1",
    date: "2026-08-29",
    notes: RELEASE_1_13_1_NOTES
  },
  {
    version: "1.13.0",
    date: "2026-08-28",
    notes: RELEASE_1_13_0_NOTES
  },
  {
    version: "1.12.13",
    date: "2026-08-28",
  ...
  },
  {
    version: "1.12.12",
    date: "2026-08-28",
    notes: RELEASE_1_12_12_NOTES
  },
  {
    version: "1.12.11",
    date: "2026-08-28",
    notes: RELEASE_1_12_11_NOTES
  },
  {
    version: "1.12.10",
    date: "2026-08-27",
    notes: RELEASE_1_12_10_NOTES
  },
  {
    version: "1.12.9",
    date: "2026-08-27",
  ...
  },
  {
    version: "1.11.2",
    date: "2026-08-21",
    notes: RELEASE_1_11_2_NOTES
  },
  {
    version: "1.11.1",
    date: "2026-08-21",
    notes: RELEASE_1_11_1_NOTES
  },
  {
    version: "1.11.0",
    date: "2026-08-21",
    notes: RELEASE_1_11_0_NOTES
  },
  {
    version: "1.10.0",
    date: "2026-08-20",
  ...
  },
  {
    version: "1.9.4",
    date: "2026-08-10",
    notes: RELEASE_1_9_4_NOTES
  },
  {
    version: "1.9.3",
    date: "2026-08-10",
    notes: RELEASE_1_9_3_NOTES
  },
  {
    version: "1.9.2",
    date: "2026-08-08",
    notes: RELEASE_1_9_2_NOTES
  },
  {
    version: "1.9.1",
    date: "2026-08-07",
    notes: RELEASE_1_9_1_NOTES
  },
  {
    version: "1.9.0",
    date: "2026-08-07",
    notes: RELEASE_1_9_0_NOTES
  },
  {
    version: "1.8.4",
    date: "2026-07-29",
    notes: RELEASE_1_8_4_NOTES
  },
  {
    version: "1.8.3",
    date: "2026-07-29",
    notes: RELEASE_1_8_3_NOTES
  },
  {
    version: "1.8.2",
    date: "2026-07-29",
  ...
  },
  {
    version: "1.8.0",
    date: "2026-07-28",
    notes: RELEASE_1_8_0_NOTES
  },
  {
    version: "1.7.6",
    date: "2026-07-28",
    notes: RELEASE_1_7_6_NOTES
  },
  {
    version: "1.7.5",
    date: "2026-07-28",
    notes: RELEASE_1_7_5_NOTES
  },
  {
    version: "1.7.4",
    date: "2026-07-24",
  ...
  },
  {
    version: "1.7.1",
    date: "2026-07-23",
    notes: RELEASE_1_7_1_NOTES
  },
  {
    version: "1.7.0",
    date: "2026-07-23",
    notes: RELEASE_1_7_0_NOTES
  },
  {
    version: "1.6.1",
    date: "2026-07-22",
  ...
  },
  {
    version: "0.10.2",
    date: "2026-07-11",
    notes: RELEASE_0_10_2_NOTES
  },
  {
    version: "0.10.1",
    date: "2026-07-11",
    notes: RELEASE_0_10_1_NOTES
  },
  {
    version: "0.10.0",
    date: "2026-07-10",
  ...
  },
  {
    ...
    notes: [
      ...
      "频道创建者可为频道指定图标，管理员可在频道面板更新图标、名称和描述。",
  ...
  },
  {
    version: "0.2.7",
    date: "2026-06-15",
    notes: ["修复语音已听标记请求被空 JSON body 拒绝的问题。", "保留 0.2.6 的 iPhone 播放顺序修复、低码率 M4A 转码和语音气泡收窄。"]
  },
  {
    version: "0.2.6",
    date: "2026-06-15",
    notes: ["修复 iPhone 点击语音播放无反应的问题，播放会直接响应用户点击。", "服务端统一把语音压缩为 16kHz 单声道低码率 AAC/M4A。", "语音气泡变窄，波形竖条更细更密。"]
  },
  {
    version: "0.2.5",
    date: "2026-06-15",
    notes: ["整理维护说明入口，方便后续环境管理。", "保留 0.2.4 语音消息重做：波形气泡、低码率发送和个人未听红点。"]
  },
  {
    version: "0.2.4",
    date: "2026-06-15",
    notes: ["语音消息改为 WhatsApp 风格气泡，显示播放按钮、波形预览、时长和体积。", "录音发送使用更低码率，并在服务端优先压缩为跨设备更友好的 m4a。", "未收听语音显示小红点，收听状态按当前账号单独记录，不影响其他用户。", "发送前语音预览也改为波形样式，可直接播放、删除或发送。"]
  },
  {
    version: "0.2.3",
    date: "2026-06-15",
    notes: ["接龙创建改为应用内窗口，不再使用浏览器弹窗。", "参与接龙改为确认提示，并始终从最新接龙名单继续追加群内昵称。", "桌面端左右栏都支持收起与展开。", "移除历史消息置顶入口，置顶公告仅能由管理员在控制面板设置。"]
  },
  {
    version: "0.2.2",
    date: "2026-06-15",
    notes: ["补充项目维护说明。", "明确环境配置需要单独管理。", "整理发布前检查说明。"]
  },
  {
    version: "0.2.1",
    date: "2026-06-15",
    notes: ["修复桌面端打开成员面板时聊天区被遮罩变暗的问题。", "置顶消息支持展开与折叠，并可从展开区定位到原消息。", "输入栏左侧改为语音录制，支持录音、停止、预览和发送。", "输入栏右侧加号改为功能面板，可添加文件、照片和发起接龙。"]
  },
  {
    version: "0.2.0",
    date: "2026-06-15",
    notes: ["建立新版聊天室测试基线：Vue 3、Fastify、Socket.IO、Prisma、MySQL。", "完成微信式响应布局，覆盖桌面、平板、手机和 iPhone Safari/PWA 关键布局。", "加入引用回复、接龙新消息、置顶消息、附件、成员与虚拟角色管理面板。", "预留虚拟角色 engine gateway，支持事件拉取、幂等 action、typing、记忆和多条发送。", "完成旧版 SQLite 数据迁移流程。"]
  }
] as const;
