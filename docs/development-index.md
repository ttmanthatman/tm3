...
- `src/server/sermon/` and `src/server/routes/sermon.ts`: sermon verse presentation. `SermonPresentationService` (`presentations.ts`) keeps one concurrent presentation per presenter account — each with its own queue, current item, annotations, and audience-wide `display` settings (font family, fontScale, marginPct, background) — plus an account→presentation seat map enforcing single-seat mutual exclusion. Each presenter gets an in-memory `SermonStateStore` persisted to a `Setting` key-value row `sermon.presentation.{accountId}` (no schema change; the legacy global `sermon.presentation` row migrates to its presenter key at startup, cleared rows are deleted on restore, and rows with a flat `fontScale` or missing queue-item `kind` still migrate on deserialize). Presentations have a `scope`: `group` (any registered account may start, invitees only) or `assembly` (requires the presenter grant, re-checked with a fresh account read at start, everyone may join). `sermon:*` mutation events operate on the caller's own presentation and broadcast `sermon:state` room-targeted to `sermon:{presenterAccountId}`; `sermon:start/join/leave/invite/remove-viewer/end` manage lifecycle (join is exclusive with `seated-elsewhere` rejections, invites validate account existence and unseated targets, presenters remove viewers, admins force-end anyone). Audience membership is volatile: the account's last-socket disconnect releases its seat server-side while the presentation itself survives. Directory changes broadcast globally as `sermon:directory`, with a `GET /api/sermon/directory` fallback. Queue items carry ordered `blocks` (passage slices into the flat `verses` array, or free-text) plus `source` (original input for hot re-editing) and `scrollLines` (audience-synced in-slide scroll offset); reference blocks resolve through the Bible lookup, unrecognized references degrade to text blocks instead of being dropped. `sermon:add` takes parsed slides, `sermon:update` re-parses one item in place (annotations and scroll reset), `sermon:scroll` sets the synced line offset, and legacy `sermon:add-text` remains for old clients. A connection snapshot re-joins the room and pushes state while a seat is held — the presenter's full queue, the audience's active state only. `/api/sermon/presenter-status` reports the per-account presenter grant (`Account.sermonPresenterUntil`, admins always allowed); the presenter-gated `/api/sermon/accounts` lists registered accounts with online status and `seatedPresentation` for the audience picker (guests filtered out); `/api/messages/:id/sermon-request/decide` lets administrators approve or reject `sermon_request` message cards.
- `src/client/features/sermon/`: client side of sermon verse presentation. `useSermon.ts` owns the shared sermon state (fed by `sermon:state`/`sermon:request:decided`/`sermon:directory`/`sermon:invited`/`sermon:removed`/`sermon:ended` subscriptions in `store.ts`'s `connectSocket()`): the presentation directory, live invites, the presentation the account is joined to (`joinedPresentationId`, null when only hosting — a pushed `sermon:state` applies only when it matches the joined or own presentation), removed/ended notices, and the presenter-gated watch-account list; it also wraps socket emits with ACK-timeout handling and exposes the phase-2 actions `start`/`join`/`leave`/`invite`/`removeViewer`/`end` (optimistic seat changes roll back on failure, `seated-elsewhere` rejections surface a “leave current and join” flow); `sermonInput.ts` parses the unified composer input into slides (`parseSermonInput`, reusing the chat Bible-reference detector): scripture stays scripture, free text stays text, Enter is an in-slide newline, and the “每处经文一屏” option splits each detected reference into its own slide; `sermonText.ts` splits verse text into annotation segments by plain-text offsets and free-text bodies into paragraphs; `sermonDisplay.ts` maps `state.display` to CSS variables and `data-sermon-*` attributes on the `.sermon-overlay` stage root (custom hex backgrounds pick their foreground by relative luminance). `SermonStage.vue` is the shared scripture/text stage (reference badge, presenter credit, annotation segments, ordered passage/text blocks; legacy items without `blocks` render from `verses`/`content`) used by the audience overlay, the presenter presentation view and the desktop preview panes so all ends stay identical; it also applies the synced `scrollLines` offset to its scroll body (each end converts lines with its own line height); typography comes from the stage-root CSS variables, and margins are percentage padding inside the card so they work past the 880px cap. `SermonWorkspace.vue` hosts the start screen (scope picker — group only without the grant — plus the audience selector from `/api/sermon/accounts` with online dots and seated accounts grayed out), the unified composer (live parse preview with debounced lookup feedback and on-page shortcut hints), queue management with presenter audience management (current viewers, remove), the presenter view with Shift+↑/↓ line scrolling and “编辑本屏” hot editing, and the desktop preview column. `SermonDisplayControls.vue` is the shared display-settings control group (mobile present footer and desktop left column). `SermonHub.vue` sits in the chat header with a watchable-count badge, stacked invite banners with Join / Later / mute, and removed/ended toasts; `sermonHub.ts` computes the watchable list (invited group presentations plus active assemblies; while seated elsewhere, group entries offer “leave current and join” and assembly entries show a disabled “available after leaving” state) and the per-account muted set persists in localStorage. `SermonOverlay.vue` (audience, mounted only while the joined presentation is active, minimizable to a corner bar) and `SermonRequestCard.vue` (rendered for `sermon_request` messages, admin approve/reject) are `defineAsyncComponent` chunks so inactive users never download that sermon code.
