...
- `src/server/routes/unreadCounts.ts`: batched unread-seed endpoint (`/api/messages/unread-counts`). It validates the client's lastRead map, intersects requested channels with the shared channel-visibility rule (`channelListWhere` in `index.ts`), and returns one grouped per-channel count of messages not sent by the requester.
