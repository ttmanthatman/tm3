...
- `src/server/bible/` and `src/client/bibleReferences.ts`: Bible lookup and reference parsing. `src/server/bible/workspaceState.ts` cleans the account-level reader workspace snapshot stored under the `workspace` key of `Account.biblePreferences` (via `PATCH /api/me/preferences`); `src/server/routes/bible.ts` owns `POST /api/bible/share`, which posts a `bible_session` message whose payload records the open pane layout. Client side, `src/client/bibleWorkspaceState.ts` persists workspace state locally and extracts/restores the server snapshot, `src/client/bibleSessionShare.ts` builds/parses the share payload, and `src/client/features/bible/BibleSessionCard.vue` renders the chat card that reopens the shared pane layout locally.
