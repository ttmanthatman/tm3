...
- `src/server/routes/wechatRelay.ts` and `src/client/features/admin/WeChatRelayPanel.vue`: administrator-managed NAS WeChat relay control plane. The server persists the selected source channel, target label, safe start cursor, and one pending bind/test action in the generic settings table; the device uses a dedicated bearer token for outbound polling and reports only heartbeat, queue, binding, action, and error status.
