...
状态：P0、S0–S3 已完成。核对日期：2026-09-23。

S3 按本计划只提供可注入的可靠发送协调器、实时播放资格和有界缓存，还没有生产画板或消息卡片。用户可见的真实手写入口与播放界面仍需 K1–K4，完成前不得宣称整项功能已经上线。
...
| 实施前基线基于 v1.13.2 | 版本描述已旧；本文审查时还没有 handwriting 类型，现已由 S0–S3 补齐协议、枚举、服务端和客户端基础设施 |
...
### R0 自动化复核记录（2026-09-23）

- 新增 `e2e/tests/handwriting.spec.ts`：两个独立账号/浏览器上下文从真实画板发送两个字（含尚未点“完成此字”的最后一字），收件端捕获多帧实时播放；刷新后显示静态成品并可手动重播；随后完成 Socket 重连和撤回。画板弹窗另检查 360/390/1280px 不越出视口。
- 丢 ACK 用例让服务端真实落库，但发送端同时丢弃本人的 `message:new` 和 ACK；界面保留未确认快照，点击重试沿用原 `clientRequestId`，刷新后按该 UUID 只存在一条消息。
- 首次压力复现中，`handwriting-s2.spec.ts --repeat-each=10` 有 2 次第一条 `message:send` ACK 超时。trace 证明浏览器在服务端异步连接初始化完成、注册 `message:send` 监听器之前发出了事件。服务端现在仅在所有入站监听器注册后发出 `session:ready`，客户端收到它后才把连接标为可发送；同一压力用例修复后连续 20/20 通过。
- 隔离库联合验收 `handwriting.spec.ts + handwriting-s2.spec.ts + smoke.spec.ts` 为 16/16 通过；从空的 `tm3_migration_verify` 应用全部 18 个迁移、状态检查和 schema diff 均通过。
- `npm run verify:full` 通过：公开树与发布一致性检查、lint（0 error，保留仓库已有 warning）、客户端/服务端类型检查、171 个测试文件和生产构建均完成。
- 仍需人工/专门环境验收：iPhone WebKit 与 iOS 真机手指/旋转/后台恢复、真实 WebSocket 丢包、50/200 条密集手写历史的传输/解析/longtask/滚动测量，以及真人 2/10/30 字样本。上述项目不能由本轮 Chromium viewport 或 Socket 回调模拟替代。

### R1 彩色笔画与移动端热路径复核（2026-09-23）

- 笔画协议增加固定 8 色可选值；旧笔画缺少颜色时继续按默认墨色显示，默认色在规范化时省略，非法颜色在落库前拒绝。颜色跟随单笔冻结，发送预览、实时播放、历史、草稿和幂等 hash 都复用同一规范化 payload。
- 移除顶部重复的“已完成的字”Canvas 区，整字删除入口合并到底部发送预览。当前画板从每个采样点全量重画改为只追加新增点；底部预览只在抬笔或编辑动作后刷新。
- 草稿保存从每个原始/coalesced pointer sample 都克隆、规范化并写 IndexedDB，改为只在采样点实际被接纳时触发、300ms 窗口合并，并在抬笔时立即 flush。纯函数回归用例验证 200 次更新只排一个待保存任务，追加单点只执行常数数量 Canvas 操作。
- 这些自动化信号证明重复工作已被移出触控热路径，但不等同于 iPhone 真机温升、耗电或帧耗时测量；上述 R0 真机和密集历史验收项仍保留。
