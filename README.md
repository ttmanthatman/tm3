...
访问 [demo.xiaogushi.us](https://demo.xiaogushi.us) 可以直接体验当前版本。演示站使用 20 个虚构用户和专门制作的示例内容，不包含正式站的用户资料或聊天记录；登录凭据由维护者单独提供，不写入公开仓库。

建议先进入“功能问答”频道：里面用一问一答的方式介绍了消息撤回与点赞、代祷卡片、推送通知、音乐与歌谱、良友节目、圣经阅读和查找、动态背景、置顶消息、AI 助手、私聊与小群等功能。其他频道还准备了图片、文件、语音、视频、歌曲引用、经文展开、回复、置顶以及全部消息特效，方便按真实聊天流程逐项体验。
...
演示模式内置在主程序中，但默认完全关闭：普通部署不会请求 GitHub、下载演示数据或加载演示管理界面。新建专用演示站时，在部署命令中增加 `DEMO_MODE=1`：

```bash
sudo env DOMAIN=demo.example.com EMAIL=you@example.com DEMO_MODE=1 bash deploy-vps.sh
```

已经部署好的服务器，在项目目录中运行一次下面的命令即可写入服务器设置，并在使用 PM2 时自动重启正确的应用：
...
| `DEMO_MODE` | 是否在服务器端开放按需演示数据载入与复位，默认关闭 |
| `DEMO_MANIFEST_URL` | 可选的 GitHub 演示数据清单地址；通常无需设置 |
