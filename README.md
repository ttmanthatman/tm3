...
- 消息按窗口分页加载，进入频道先显示最近消息，向上滚动再加载历史记录，避免大量历史消息一次性撑满 DOM 和内存。
...
- `/震动`：气泡持续轻微摇晃。
- `/飞机`：文字在聊天区横向循环飞行。
- `/光芒万丈`：消息气泡向外放射太阳般的光芒，支持 WebGL 粒子光束增强。
- `/跑马灯`：气泡外圈出现独立彩灯追逐，适合节日灯牌氛围。
- `/水波`：气泡呈现侧视水槽效果，带液面、波峰和高光；手机倾斜时水面会跟随移动，桌面端鼠标划过会搅动水波。
- `/水滴`：气泡底部凝结水滴，液滴带重力下落，碰到其他消息气泡会溅开水花，并支持 WebGL 水滴增强。
- `/下雨`：一次性聊天室天气效果。消息发出后整个聊天区下 15 秒大雨；同一条消息不会因为再次点击而重播，播放中点击也不会中断。

0.6.0 起，水体、天气、液滴和光效采用 Three.js/WebGL 透明特效层增强。特效层只测量并渲染当前可见的消息气泡，配合分页消息窗口避免大量历史消息拖慢渲染；不支持 WebGL 或开启减少动态效果时，界面仍保留轻量 CSS 回退。
